...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
# Load data
raw.data <- read.table("../data/Raw_data.txt", sep="\t", header = T, row.names = 1)
raw.data<-raw.data[1:(nrow(raw.data)-7),]
colnames(raw.data) <- sub("_S.*", "", colnames(raw.data))
metadata <- read.table("../data/AB_Mapping_file.txt", sep="\t", header = T)
...
ts <- read.table("../data/TS_Male_Female.csv", header = T, sep = "\t")
...
plot.lfc.lhm <- plot.graph.lfc(population = "LHm", yli = 0.2, ylabyn = T,
                               y1 = 0.15, y2 = 0.15, y3 = 0.19)
plot.lfc.dah <- plot.graph.lfc(population = "Dahomey", yli = 2.8, ylabyn = F,
                               y1 = 2, y2 = 2, y3 = 2.5)
...
get.p((b.con.samples$Male_UB)),
get.p((b.con.samples$Female_UB)))
...
#### P <0.05
```{r, warning=FALSE, message=FALSE, echo=FALSE}
intercept <- m.dah.TS$coefficients["(Intercept)"]
mcmc_areas(m.dah.TS, transformations = list("groupFemaleMale_biased" = function(x) x + intercept,
                                         "groupFemaleUnbiased" = function(x) x + intercept,
                                         "groupMaleFemale_biased" = function(x) x + intercept,
                                         "groupMaleMale_biased" = function(x) x + intercept,
                                         "groupMaleUnbiased" = function(x) x + intercept))
...
plot.cv <- function(vstdata, ylimi, population){
  ...
  p1 <- ggplot(vstdata, aes(x = variable, y = value)) +
    ...
    geom_hline(data=data.frame(value= median(subset(vstdata, variable == "A1" & sex == "Female")$value), sex="Female"), aes(yintercept=value),linetype="longdash", color = "red", size=0.5) +
    geom_hline(data=data.frame(value= median(subset(vstdata, variable == "A1" & sex == "Male")$value), sex="Male"), aes(yintercept=value),linetype="longdash", color = "red", size=0.5) +
...
p <- ggarrange(p1, p2,
               ...
               widths = 1, heights = 1,
               common.legend = F)
save_plot("Figures/Coeff_var.pdf", p, base_width = 10, base_height = 3)
p
```
## CV testing

```{r}
bayes.cv.test.group <- function(dataset){
  dataset <- dataset[complete.cases(dataset),]
  dataset$group <- factor(paste0(dataset$sex, dataset$variable))
  print(head(dataset))
  m <- stan_glm(value ~ variable + sex, data = dataset, cores = 8, seed = 100, refresh = 0)
  return(m)
}

bayes.contrasts.cv.group <- function(m){
  b.con <- as.data.frame(m)
  b.con <- b.con[,c(1:5)]
  colnames(b.con)[1] <- c("Intercept")
  
  b.con$FemaleA1 <- b.con$Intercept 
  b.con$FemaleA2 <-  b.con$Intercept + b.con$groupFemaleA2
  b.con$MaleA1 <-  b.con$Intercept + b.con$groupMaleA1
  b.con$MaleA2 <-  b.con$Intercept + b.con$groupMaleA2
  return(b.con)
}

contr.cv.bay <- function(model, pvalue){
  b.con <- bayes.contrasts.cv.group(model)
  b.con.h <- hypothesis(b.con, 
                          c("FemaleA2 - FemaleA1 = 0",
                            "MaleA2 - MaleA1 = 0"),
                             alpha = pvalue)
  b.con.h <- as.data.frame(b.con.h$hypothesis)
  print(b.con.h)
...
m.var.lhm <- bayes.cv.test.group(var.lhm)
m.var.dah <- bayes.cv.test.group(var.dah)

intercept <- m.var.lhm$coefficients["(Intercept)"]
mcmc_areas(m.var.lhm, transformations = list(
                                         "sexMale" = function(x) x + intercept,
                                         "variableA2" = function(x) x + intercept), prob=0.9995)

intercept <- m.var.lhm$coefficients["(Intercept)"]
mcmc_areas(m.var.lhm, transformations = list("groupFemaleA2" = function(x) x + intercept,
                                         "groupMaleA1" = function(x) x + intercept,
                                         "groupMaleA2" = function(x) x + intercept), prob=0.9995)

intercept <- m.var.dah$coefficients["(Intercept)"]
mcmc_areas(m.var.dah, transformations = list("groupFemaleA2" = function(x) x + intercept,
                                         "groupMaleA1" = function(x) x + intercept,
                                         "groupMaleA2" = function(x) x + intercept))

b.con.samples.lhm <- bayes.contrasts.cv.group(m.var.lhm)
b.con.samples.dah <- bayes.contrasts.cv.group(m.var.dah)

b.con.h.lhm <- contr.cv.bay(model = m.var.lhm,
                     pvalue = 0.0005)
b.con.h.dah <- contr.cv.bay(model = m.var.dah,
                     pvalue = 0.0005)

as.data.frame(hypothesis(b.con.samples.lhm, c("FemaleA2 - FemaleA1 = 0", "MaleA2 - MaleA1 = 0"), alpha = 0.0005)$hypothesis)

b.con.h.lhm$Pmcmc <- c(get.p(b.con.samples.lhm$FemaleA2 - b.con.samples.lhm$FemaleA1),
                   get.p((b.con.samples.lhm$MaleA2 - b.con.samples.lhm$MaleA1)))
b.con.h.dah$Pmcmc <- c(get.p(b.con.samples.dah$FemaleA2 - b.con.samples.dah$FemaleA1),
                       get.p((b.con.samples.dah$MaleA2 - b.con.samples.dah$MaleA1)))
...
# Are males the more sensitive sex – do SB genes change more in males?
...
for (i in 1:number){
  ...
  rerun_Dah<-stan_glm(Foc_off~Sex*as.factor(Age)+offset(log(Foc_Par))+offset(log(Comp_off/Comp_Par)) + Cage, family =poisson, 
                        data=TheRun,
                        seed=100,cores=4,iter=4000,adapt_delta = 0.95,
                        prior_intercept = normal(location = 0, scale = 10),
                        prior = normal(location = 0, scale = 2.5),
                        warmup=1000)
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
df <- read.table(file = "../data/Mortality2.txt", sep = "\t", header = T)
df2 <- df[,c(1,2,8,9)]
